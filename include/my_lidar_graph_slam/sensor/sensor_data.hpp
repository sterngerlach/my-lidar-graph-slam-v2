...
public:
    ...
    /* Set the minimum range in meters */
    void SetMinRange(const T minRange) { this->mMinRange = minRange; }
    /* Set the maximum range in meters */
    void SetMaxRange(const T maxRange) { this->mMaxRange = maxRange; }
    /* Set the minimum angle in radians */
    void SetMinAngle(const T minAngle) { this->mMinAngle = minAngle; }
    /* Set the maximum angle in radians */
    void SetMaxAngle(const T maxAngle) { this->mMaxAngle = maxAngle; }
    ...
    /* Retrieve the angle data */
    inline std::vector<T>& Angles() { return this->mAngles; }
    /* Retrieve the range data */
    inline std::vector<T>& Ranges() { return this->mRanges; }
...
private:
    ...
    const RobotPose2D<T> mOdomPose;
    /* Robot velocity (optional) */
    const RobotPose2D<T> mVelocity;
    /* Relative sensor pose in robot frame */
    const RobotPose2D<T> mRelativeSensorPose;
