...
/*
 * EdgePose struct stores the relative pose with its uncertainty
 * (information matrix) using Eigen data types for pose graph optimization
 */
struct EdgePose final
{
    /* Constructor */
    EdgePose(const bool isLoopConstraint,
             const int localMapNodeIdx,
             const int scanNodeIdx,
             const Eigen::Vector3d& relativePose,
             const Eigen::Matrix3d& informationMat) :
        mIsLoopConstraint(isLoopConstraint),
        mLocalMapNodeIdx(localMapNodeIdx),
        mScanNodeIdx(scanNodeIdx),
        mRelativePose(relativePose),
        mInformationMat(informationMat) { }
    /* Destructor */
    ~EdgePose() = default;

    /* Flag to represent whether this edge is a loop constraint */
    const bool            mIsLoopConstraint;
    /* Index of the local map node */
    const int             mLocalMapNodeIdx;
    /* Index of the scan node */
    const int             mScanNodeIdx;
    /* Relative pose between two pose graph nodes */
    const Eigen::Vector3d mRelativePose;
    /* Information matrix (inverse of the covariance matrix) */
    const Eigen::Matrix3d mInformationMat;
};
