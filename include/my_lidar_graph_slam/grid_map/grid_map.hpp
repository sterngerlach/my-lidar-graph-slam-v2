...
public:
    ...
    void Expand(const double localMinX,
                ...
                const double localMaxX,
                const double localMaxY,
                const double enlargeStep = 0.1);

    /* Reset occupancy probability values of all grid cells */
    void Reset();

    /* Get the unknown occupancy probability value */
    inline ValueType UnknownValue() const override
    { return GridCellType::Unknown; }
    /* Get the internal value to represent unknown occupancy probability */
    inline StorageType UnknownRawValue() const override
    { return GridCellType::UnknownRaw; }
    /* Get the minimum internal value */
    inline StorageType RawValueMin() const override
    { return GridCellType::ValueMin; }
    /* Get the maximum internal value */
    inline StorageType RawValueMax() const override
    { return GridCellType::ValueMax; }
    /* Get the internal value range */
    inline StorageType RawValueRange() const override
    { return GridCellType::ValueRange; }
    ...
    /* Get the internal value of the specified grid cell */
    StorageType RawValue(const Point2D<int>& gridCellIdx) const override
    ...
    /* Get the internal value of the specified grid cell */
    StorageType RawValue(const Point2D<int>& gridCellIdx,
                         const StorageType defaultVal) const override
    ...
    /* Set the occupancy probability value of the specified grid cell */
    void SetValue(const Point2D<int>& gridCellIdx,
                  const ValueType probValue)
    { this->SetValue(gridCellIdx.mX, gridCellIdx.mY, probValue); }
    ...
    /* Set the internal value of the specified grid cell */
    void SetRawValue(const Point2D<int>& gridCellIdx,
                     const ValueType rawValue)
    { this->SetRawValue(gridCellIdx.mX, gridCellIdx.mY, rawValue); }
    ...
    /* Convert the grid cell index to the offset inside the patch */
    Point2D<int> GridCellIndexToPatchOffset(int idxX, int idxY) const;
    /* Convert the grid cell index to the offset inside the patch */
    Point2D<int> GridCellIndexToPatchOffset(
        const Point2D<int>& gridCellIdx) const
    { return this->GridCellIndexToPatchOffset(
        gridCellIdx.mX, gridCellIdx.mY); }
                    ...
                    const Point2D<double>& localCenterPos)
{
    ...
    /* Empty grid map with no patch is allowed */
    const double actualMapSizeX = std::max(0.0, mapSizeX);
    const double actualMapSizeY = std::max(0.0, mapSizeY);

    /* Calculate and validate the number of patches */
    const double patchSizeInMeters = patchSize * this->mResolution;
    this->mNumOfPatchesX = static_cast<int>(std::ceil(
        actualMapSizeX / patchSizeInMeters));
    this->mNumOfPatchesY = static_cast<int>(std::ceil(
        actualMapSizeY / patchSizeInMeters));
...
void GridMap<T>::SetValue(const int idxX, const int idxY,
                          const ValueType probValue)
{
    this->GridCellAt(idxX, idxY).SetValue(probValue);
}
...
void GridMap<T>::SetRawValue(const int idxX, const int idxY,
                             const StorageType rawValue)
{
    this->GridCellAt(idxX, idxY).SetRawValue(rawValue);
}
...
/* Convert the grid cell index to the offset inside the patch */
template <typename T>
Point2D<int> GridMap<T>::GridCellIndexToPatchOffset(
    int idxX, int idxY) const
{
    assert(this->mPatchSize > 0);
    return Point2D<int> { idxX % this->mPatchSize,
                          idxY % this->mPatchSize };
}
...
{
    assert(this->PatchIsInside(patchIdxX, patchIdxY));
    return this->mPatches.get() + patchIdxY * this->mNumOfPatchesX + patchIdxX;
}
