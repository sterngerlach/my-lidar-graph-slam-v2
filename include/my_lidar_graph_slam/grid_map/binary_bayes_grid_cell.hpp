...
template <typename T>
class BinaryBayesGridCell final : public GridCell<T, T, double>
{
public:
    /* Base type */
    using BaseType = GridCell<T, T, double>;

    /* Default constructor */
    BinaryBayesGridCell() : BaseType(),
                            mValue(Unknown) { }
    /* Destructor */
    ...
    /* Clamp an occupancy probability value */
...
/* Set the value of the grid cell */
template <typename T>
void BinaryBayesGridCell<T>::SetValue(const T newValue)
{
...
/* Set the internal value of the grid cell */
template <typename T>
void BinaryBayesGridCell<T>::SetRawValue(const T newRawValue)
{
